...
const { generateShortLivedToken } = require('@company-local-ai/api');
const { Tools, EToolResources } = require('company-local-ai-data-provider');
...
const createFileSearchTool = async ({ userId, files, entity_id, fileCitations = false }) => {
  return tool(
    async ({ query }) => {
      ...
      /**
       *
       * @param {import('company-local-ai-data-provider').TFile} file
