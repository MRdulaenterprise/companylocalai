...
const { isEnabled, checkEmailConfig, isEmailDomainAllowed } = require('@company-local-ai/api');
const { ErrorTypes, SystemRoles, errorsToString } = require('company-local-ai-data-provider');
const {
  findUser,
  findToken,
  createUser,
...
const sendVerificationEmail = async (user) => {
  ...
  await sendEmail({
    ...
    payload: {
      appName: process.env.APP_TITLE || 'Company Local AI',
...
const setAuthTokens = async (userId, res, _session = null) => {
  try {
    ...
    res.cookie('token_provider', 'company-local-ai', {
      expires: new Date(refreshTokenExpires),
      httpOnly: true,
      secure: isProduction,
      sameSite: 'strict',
