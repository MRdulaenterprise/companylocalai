...
const { extractEnvVariable, STTProviders } = require('company-local-ai-data-provider');
...
class STTService {
  ...
  async getProviderSchema(req) {
    ...
    if (!sttSchema) {
      throw new Error(
        'No STT schema is set. Did you configure STT in the custom config (company-local-ai.yaml)?',
      );
    }
