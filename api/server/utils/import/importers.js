...
const { EModelEndpoint, Constants, openAISettings, CacheKeys } = require('company-local-ai-data-provider');
...
function getImporter(jsonData) {
  ...
  // For Company Local AI
  if (jsonData.conversationId && (jsonData.messagesTree || jsonData.messages)) {
    logger.info('Importing Company Local AI conversation');
    return importCompany Local AIConvo;
  }
...
/**
 * Imports a Company Local AI conversation from JSON.
 *
 ...
 * @returns {Promise<void>} - A promise that resolves when the import is complete.
 */
async function importCompany Local AIConvo(
  jsonData,
  requestUserId,
  builderFactory = createImportBatchBuilder,
) {
  try {
    ...
    if (jsonData.recursive) {
      ...
      throw new Error('Invalid Company Local AI file format');
    }
    ...
    logger.error(`user: ${requestUserId} | Error creating conversation from Company Local AI file`, error);
  }
}
