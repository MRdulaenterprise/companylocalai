...
(async () => {
  ...
  if (docker) {
    ...
    console.purple('Pruning all Company Local AI Docker images...');

    const imageName = singleCompose ? 'company-local-ai_single' : 'company-local-ai';
    try {
      execSync(`${sudo}docker rmi ${imageName}:latest`, { stdio: 'inherit' });
    } catch (e) {
      console.purple('Failed to remove Docker image company-local-ai:latest. It might not exist.');
    }
    console.purple('Removing all unused dangling Docker images...');
    execSync(`${sudo}docker image prune -f`, { stdio: 'inherit' });
    console.purple('Building new Company Local AI image...');
  ...
  console.green('Your Company Local AI app is now up to date! Start the app with the following command:');
  console.purple(startCommand);
  console.orange(
    "Note: it's also recommended to clear your browser cookies and localStorage for Company Local AI to assure a fully clean installation.",
  );
