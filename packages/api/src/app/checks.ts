import { logger, webSearchKeys } from '@company-local-ai/data-schemas';
import { Constants, extractVariableName } from 'company-local-ai-data-provider';
...
const deprecatedVariables = [
  {
    key: 'CHECK_BALANCE',
    description:
      'Please use the `balance` field in the `company-local-ai.yaml` config file instead.\nMore info: https://company-local-ai.ai/docs/configuration/company-local-ai_yaml/object_structure/balance#overview',
  },
  {
    key: 'START_BALANCE',
    description:
...
function checkPasswordReset() {
  ...
  if (!emailEnabled && passwordResetAllowed) {
    logger.warn(
      ...
      https://www.company-local-ai.ai/docs/configuration/authentication/email
...
export function checkVariables() {
  ...
  if (hasDefaultSecrets) {
    ...
    https://www.company-local-ai.ai/toolkit/creds_generator
...
Latest version: ${Constants.CONFIG_VERSION}
      ...
      https://www.company-local-ai.ai/changelog\n\n`,
    );
  }
}
    ...
    if (typeof value === 'string') {
      ...
      if (varName) {
        ...
        logger.warn(
          ...
          This is incorrect! You should use environment variable references in your company-local-ai.yaml file, such as:
          ${key}: "\${YOUR_ENV_VAR_NAME}"
          
          Then set the actual API key in your .env file or environment variables.
          
          More info: https://www.company-local-ai.ai/docs/configuration/company-local-ai_yaml/web_search`,
        );
      }
    }
  });
