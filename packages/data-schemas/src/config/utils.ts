 ...
 * 2. If running within Company Local AI monorepo (when cwd ends with /api), use api/logs
...
export const getLogDirectory = (): string => {
  ...
  // For Company Local AI project structure, use api/logs
  // For external consumers, they should set LIBRECHAT_LOG_DIR
  if (cwd.includes('Company Local AI')) {
    return apiLogsPath;
  }
